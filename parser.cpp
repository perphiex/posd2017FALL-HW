...
Parser::Parser(Scanner scanner)
    : _scanner(scanner), _global(Global::getInstance()) {}

Term* Parser::createTerm() {
    _currentToken = _scanner.nextToken();
    if (_currentToken.second == _global->VARIABLE)
        return new Variable(_currentToken.first);
    else if (_currentToken.second == _global->NUMBER)
        return new Number(stod(_currentToken.first));
    else if (_currentToken.second == _global->ATOM ||
             _currentToken.second == _global->ATOMSC) {
        Atom* atom = new Atom(_currentToken.first);
        if (_scanner.currentChar() == '(') return buildStruct(atom);
        return atom;
...
Term* Parser::buildStruct(Atom* functor) {
    ...
    if (_scanner.nextToken().second == ')') {
        if (functor->symbol() == "." && args.size() == 2 &&
            args[1]->match((_global->EMPTY_LIST)))
...
Term* Parser::buildListOrStruct() {
    ...
    if (_currentToken.second == ']') {
        for (int i = args.size() - 1; i >= 0; i--) {
            if (tail->getList() || tail->match(_global->EMPTY_LIST))
...
vector<Term*> Parser::getArgs() {
    vector<Term*> args;
    Term* term;
    if (_scanner.currentChar() == '(') _scanner.nextToken();
